...
def main():
    ...
    for n in range(ANTS):
        workers.append(Ant(screen, nest, pheroLayer))
    ...
    while frame < 1000:
        ...
        dt = clock.tick(FPS) * speed_multiplier / 100
...
if __name__ == '__main__':
    ...
    food = []
    for i in range(t):
        food.append(main())  # by Nik
        # pg.quit()
        print(f"i: {i}")
        print(f"--- total food: {sum(food)}")

    print(f"Total food over 1000 versions: {(sum(food) / t) / ANTS}")
